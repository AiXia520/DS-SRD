...
def train_distill(epoch, train_loader, train_loader2, module_list, criterion_list, optimizer, opt,cluster_result = None):
...
TotalStep {opt.total_step} Cov {cov_sum / num_RVs / num_RVs :.10f} Cov_approx {cov_approx_sum / num_RVs / num_RVs :.10f}')
        ...
        if opt.model_s in ['wrn_40_1', 'ShuffleV1', 'ShuffleV2','MobileNetV2', 'vgg8']:
            ...
            k_norm = nn.functional.normalize(feat_t[-1], dim=1)
        else:
            q_norm = nn.functional.normalize(feat_s[-1], dim=1)
            k_norm = nn.functional.normalize(feat_t[-1], dim=1)

        # local structured distillation
        loss_proto = torch.Tensor([0]).cuda()
        ...
        total_loss += opt.betac *  loss_proto
        lossline.update('loss_proto(*%s)' % opt.betac, loss_proto.item(), '.4f')
...
def cutmix_pick(input, target, logit_t, logit_s, model_t, model_s, opt):
    ...
    if opt.entropy_log != None and opt.total_step % 100 == 0:
        ...
        # ---
        entropy_ori =get_entropy_batch(input, model_t, model_s, opt.kd_T, opt.cutmix_pick_criterion,logit_t)
