...
class PRSDLoss(nn.Module):
    """PRD Loss function
    """
    def __init__(self, opt):
        super(PRSDLoss, self).__init__()
        self.embed = Embed(opt.s_dim, opt.low_dim)
        self.opt = opt

    def forward(self, f_s, f_ss, index, cluster_result, criterion_div):
        """
        Args:
        ...
        if cluster_result is not None:
            ...
            for n, (im2cluster, prototypes, density) in enumerate(
                ...
                student = torch.mm(f_s, proto_selected.t())
                teacher = torch.mm(f_ss, proto_selected.t())
...
class CRDLoss(nn.Module):
    ...
    def __init__(self, opt):
        ...
        # self.contrast = ContrastMemory(100, opt.n_data, opt.nce_k, opt.nce_t, opt.nce_m)
