...
def train_distill_pcl(epoch, train_loader, module_list, criterion_list, optimizer, opt, cluster_result = None):
    ...
    for idx, data in enumerate(train_loader):
        ...
        with torch.no_grad():
            feat_ss, logit_ss = model_ss(im_k, is_feat=True , preact=preact)

        if opt.model_s in ['wrn_40_1']:
        ...
        loss = opt.gamma * loss_cls + opt.alpha * loss_div + opt.beta * loss_kd + opt.beta * loss_proto
        # loss = opt.gamma * loss_cls + opt.alpha * loss_div + opt.beta * loss_consist
        acc1, acc5 = accuracy(logit_s, target, topk=(1, 5))
        losses.update(loss.item(), input[0].size(0))
        top1.update(acc1[0], input[0].size(0))
        top5.update(acc5[0], input[0].size(0))
        ...
        with torch.no_grad():
            for param_q, param_k in zip(model_s.parameters(), model_ss.parameters()):
                param_k.data = param_k.data * 0.99 + param_q.data * (1. - 0.99)
        # print info
        if idx % opt.print_freq == 0:
            print('Epoch: [{0}][{1}/{2}]\t'
