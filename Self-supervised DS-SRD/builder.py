...
class Builder(nn.Module):
    ...
    def __init__(self, base_encoder, base_encoder_distill, dim=128, qlen=4096, emam=0.999, temp=0.2, proj='lin', pred='none', method='npair', shuffle_bn=False, head_mul=1, sym=False, in_channels=3, small=False, distributed=False, kaiming_init=True):
        ...
        self.encoder_distill_q = base_encoder_distill(num_classes=dim * head_mul, in_channels=in_channels, small=small,
                                      kaiming_init=kaiming_init)
        # projection head
        dim_out, dim_in = self.encoder_q.fc.weight.shape
        dim_mlp = dim_in * head_mul

        dim_out_distill, dim_in_distill = self.encoder_distill_q.fc2.weight.shape
        dim_mlp_distill = dim_in_distill * head_mul
        ...
        if proj == 'mlpbn':
            ...
            self.encoder_distill_q.fc2 = nn.Sequential(nn.Linear(dim_in_distill, dim_mlp_distill), BatchNorm1d(dim_mlp_distill), nn.ReLU(),
                                          nn.Linear(dim_mlp_distill, dim_out_distill))
        ...
        if pred == 'mlpbn':
            ...
            self.pred2 = nn.Sequential(nn.Linear(dim_out, dim_mlp), BatchNorm1d(dim_mlp), nn.ReLU(), nn.Linear(dim_mlp, dim_out))
        elif pred == 'mlp':
    ...
    def forward(self, im_1, im_2, m=None, criterion=None, imix='none', alpha=1., mix_layers='0', num_aux=0, alpha2=1., temp = None, crdloss=None,beta=1.0):

        bsz = im_1.shape[0]
        _, _, index, contrast_idx = temp
        # inputmix
        if num_aux > 0:
        ...
        for s, (im_q, im_k) in enumerate(im_qk):
            ...
            if mix_layer == 0:
                ...
                _, q_mix = self.encoder_q(im_q, is_trans=True)  # queries: NxC
                feat_q, q = self.encoder_distill_q(im_q_origin, is_trans=False)  # queries: NxC
            ...
            if self.method == 'npair':
                ...
                with torch.no_grad():  # no gradient to keys
                    ...
                    feat_k, k = self.encoder_k(im_k,is_trans=True)  # keys: NxC
            ...
            if self.method == 'moco':
                ...
                # logits: Nx(N+K)
                logits_mix = torch.mm(q_mix, contrast.t())
                logits = torch.mm(q, contrast.t())
            else:
                logits = q.mm(k.t())
                logits_mix = q_mix.mm(k.t())
            # apply temperature
            if self.method != 'byol':
                logits /= self.temp
                logits_mix /= self.temp
            ...
            if self.method == 'byol':
                ...
                # loss = (2. - 2. * target_logits_mix).mean() + beta * crdloss(q, k, index.cuda(), contrast_idx.cuda())
                loss = (2. - 2. * target_logits_mix).mean() + beta * crdloss(feat_q[-1], feat_k[-1], index.cuda(), contrast_idx.cuda())
                # loss = (2. - 2. * target_logits_mix).mean() + torch.sum(-k_softmax * q_log_softmax, dim=1).mean()
            elif criterion is None:
                loss = None
            elif imix == 'none':
                loss = criterion(logits, labels).mean()
            else:
                loss = (lam * criterion(logits_mix, labels) + (1. - lam) * criterion(logits_mix, labels_aux)).mean()
                loss = loss + criterion(logits, labels).mean()
            if s == 0:
                # glogits = logits
                glogits = logits_mix
                glabels = labels
                ...
                glogits = torch.cat([glogits, logits_mix], dim=0)
