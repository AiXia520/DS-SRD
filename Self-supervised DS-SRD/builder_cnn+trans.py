...
class Builder(nn.Module):
    ...
    def __init__(self, base_encoder, dim=128, qlen=4096, emam=0.999, temp=0.2, proj='lin', pred='none', method='npair', shuffle_bn=False, head_mul=1, sym=False, in_channels=3, small=False, distributed=False, kaiming_init=True):
        ...
        if proj == 'mlpbn':
            ...
            self.encoder_q.fc2 = nn.Sequential(nn.Linear(dim_in, dim_mlp), BatchNorm1d(dim_mlp), nn.ReLU(),
                                              nn.Linear(dim_mlp, dim_out))
            self.encoder_k.fc2 = nn.Sequential(nn.Linear(dim_in, dim_mlp), BatchNorm1d(dim_mlp), nn.ReLU(),
                                              nn.Linear(dim_mlp, dim_out))
        elif proj == 'mlp':
    ...
    def forward(self, im_1, im_2, m=None, criterion=None, imix='none', alpha=1., mix_layers='0', num_aux=0, alpha2=1., temp = None, crdloss=None):
        ...
        for s, (im_q, im_k) in enumerate(im_qk):
            ...
            if mix_layer == 0:
                ...
                q = self.encoder_q(im_q_origin, is_trans=False)  # queries: NxC
            ...
            if self.method == 'byol':
                ...
                loss = (2. - 2. * target_logits_mix).mean() + crdloss(q, k, index.cuda(), contrast_idx.cuda())
