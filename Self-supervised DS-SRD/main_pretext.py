...
from models.dataset import CIFAR10InstanceSample,CIFAR100InstanceSample, ImageFolderSample
from models.crdloss import CRDLoss
...
parser.add_argument('-aa', '--arch-distill', metavar='ARCH', default='resnet34',
                    choices=model_names,
                    help='model architecture: ' +
                        ' | '.join(model_names) +
                        ' (default: resnet50)')
...
parser.add_argument('--dataset', default='cifar10', type=str, choices=['imagenet', 'imagenet_val', 'cifar10', 'cifar100', 'speech_commands', 'covtype', 'higgs', 'higgsall', 'higgs100K', 'higgs100Kall', 'higgs1M', 'higgs1Mall'],
                    help='dataset to use')
parser.add_argument('--proj', default='mlpbn1', type=str, choices=['lin', 'linbn', 'mlp', 'mlpbn', 'mlpbn1'],
                    help='projection layer')
parser.add_argument('--pred', default='mlpbn1', type=str, choices=['none', 'lin', 'linbn', 'mlp', 'mlpbn', 'mlpbn1'],
                    help='prediction layer')
parser.add_argument('--method', default='byol', type=str, choices=['npair', 'moco', 'mocon', 'byol'],
                    help='method to use')
parser.add_argument('--imix', default='imixup', type=str, choices=['none', 'imixup', 'icutmix'],
                    help='i-mix')
...
parser.add_argument('--lincls', action='store_true',default=True,
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = Builder(
        models.__dict__[args.arch],models.__dict__[args.arch_distill],
    ...
    if args.dataset in ['imagenet', 'imagenet_val']:
        ...
        train_dataset = ImageFolderSample(root=traindir, transform=train_transform)
        n_data = len(train_dataset)
        crdloss = CRDLoss(n_data).cuda(args.gpu)
        # import folder
        ...
        train_dataset =  CIFAR10InstanceSample(root=traindir,download=True,train=True,transform=train_transform)
        n_data = len(train_dataset)
        if args.arch == 'resnet50' and args.arch_distill == 'resnet34':
            args.s_dim = 512
            args.t_dim = 2048

        elif args.arch == 'resnet50' and args.arch_distill == 'resnet18':
            args.s_dim = 512
            args.t_dim = 2048

        elif args.arch == 'resnet34' and args.arch_distill == 'resnet18':
            args.s_dim = 512
            args.t_dim = 512
        args.n_data = n_data
        crdloss = CRDLoss(args).cuda(args.gpu)
        ...
        train_dataset = CIFAR100InstanceSample(root=traindir, download=True, train=True, transform=train_transform)
        n_data = len(train_dataset)
        if args.arch == 'resnet50' and args.arch_distill == 'resnet34':
            args.s_dim = 512
            args.t_dim = 2048
        ...
        train_dataset = speech_commands.SpeechCommandsDatasetInstanceSample(traindir, train_transform)
        n_data = len(train_dataset)
        crdloss = CRDLoss(n_data).cuda(args.gpu)
        ...
        train_loader = tabular.TabularDataLoaderSample(data=train_db[0], targets=train_db[1], shuffle=True, drop_last=True,
                                                 num_copies=2)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, criterion, optimizer, epoch, tb_logger,args, crdloss)
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            if is_milestone:
                ...
                }, is_milestone=True, filename='{}/checkpoint.pth'.format(args.model_name), epoch=epoch+1)
...
def train(train_loader, model, criterion, optimizer, epoch, tb_logger, args, crdloss):
    ...
    for i, (data) in enumerate(train_loader):
        ...
        if args.dataset in ['cifar10', 'cifar100', 'imagenet', 'speech_commands', 'covtype']:
            images, target, index, contrast_idx = data
        else:
            images,_= data
        ...
        if args.cos_m:
            ...
            m = None
        output, target, loss = model(im_1=images[0], im_2=images[1], m=m, criterion=criterion,
                                     imix=args.imix, alpha=args.alpha, mix_layers=args.mix_layers, num_aux=args.inputmix,
                                     alpha2=args.alpha2, temp=data, crdloss=crdloss,beta=args.beta)
...
def validate2(val_loader, model, classifier, criterion, epoch, tb_logger, args):
    batch_time = AverageMeter('Time', ':6.3f')
    losses = AverageMeter('Loss', ':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')
    top5 = AverageMeter('Acc@5', ':6.2f')
