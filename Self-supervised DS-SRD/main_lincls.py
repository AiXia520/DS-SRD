...
parser.add_argument('--start-epoch', default=80, type=int, metavar='N',
...
parser.add_argument('--pretrained', default='E:/xyl/mywork/imix-main-distill/save/cifar10/resnet50_byol_m0.999_proj_mlpbn1_pred_mlpbn1_shufflebn_imixup1.0_lr0.125_wd1.0e-04_bsz256_ep1000_cos_warm_trial0/checkpoint_200.pth', type=str,
...
parser.add_argument('--istrans', action='store_true',
                    help='transformer or cnn prediction')
best_acc1 = 0
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model.fc2.weight.data.normal_(mean=0.0, std=0.01)
    ...
    if args.pretrained:
        if os.path.isfile(args.pretrained):
            ...
            for k in list(state_dict.keys()):
                ...
                if k.startswith('module.encoder_distill_q.') and not k.startswith('module.encoder_distill_q.fc2.'):
                    # remove prefix
                    state_dict[k[len("module.encoder_distill_q."):]] = state_dict[k]

                elif k.startswith('module.encoder_distill_q.') and not k.startswith('module.encoder_distill_q.fc.'):
                    # remove prefix
                    state_dict[k[len("module.encoder_distill_q."):]] = state_dict[k]

                elif k.startswith('encoder_distill_q.') and not k.startswith('encoder_distill_q.fc.'):
                    # remove prefix
                    state_dict[k[len("encoder_distill_q."):]] = state_dict[k]

                elif k.startswith('encoder_distill_q.') and not k.startswith('encoder_distill_q.fc2.'):
                    # remove prefix
                    state_dict[k[len("encoder_distill_q."):]] = state_dict[k]
            ...
            assert set(msg.missing_keys) == {"fc.weight", "fc.bias","fc2.weight", "fc2.bias"}, set(msg.missing_keys)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            if epoch == args.epochs-1:
                ...
                }, is_best, is_milestone=True, filename='{}/checkpoint_lincls_{}_{}{}.pth'.format(os.path.dirname(args.pretrained), args.dataset, 'ft' if args.finetune else 'fr', '' if args.trial is None else '_{}'.format(args.trial)), dataset=args.dataset, epoch=epoch+1)
...
def train(train_loader, model, criterion, optimizer, epoch, tb_logger, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        # compute output
        output = model(images, is_trans= args.istrans)
...
def sanity_check(state_dict, pretrained_weights):
    ...
    for k in list(state_dict.keys()):
        ...
        if 'fc2.weight' in k or 'fc2.bias' in k or 'fc.weight' in k or 'fc.bias' in k:
            continue

        # name in pretrained model
        k_pre = 'module.encoder_distill_q.' + k[len('module.'):] \
            if k.startswith('module.') else 'module.encoder_distill_q.' + k
